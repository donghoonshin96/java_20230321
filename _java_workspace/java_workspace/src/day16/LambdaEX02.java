...
public class LambdaEX02 {
	...
	public static void main(String[] args) {
		/*
		 * Stream(스트림) : 자료의 대상과 관계없이 동일한 연산을 수행할 수 있도록 해주는 반복자 요소가 하나씩 흘러가는 형태로 처리 스트림을
		 * 생성하여 연산을 수행하면 스트림은 소모가 됨. 재사용 불가 -> 다른 연산을 하려면 스트림을 다시 생성해야 됨. 스트림 연산은 기존 자료를
		 * 변경하지 않음. 중간연산 : filter(조건에 맞는 요소 추출), distinct(중복제거), sorted(정렬), map(추출)
		...
		System.out.println("stream으로 구성한 sum:" + sum2);
		...
		int listSum = list.stream().filter(b->b>=70)
